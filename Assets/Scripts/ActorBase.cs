...
public class ActorBase : MonoBehaviour
{
	...
	private ActorBodyPoints actorBodyPoints;
	private EmActorState myState = EmActorState.Idle;
	private float stateTimer;
	private float stateDuration;
	public ActorConf actorConf;
	public WeaponConf weaponConf;
	...
	public void Init(int actorId, EmActorType _actorType, int weaponId)
	{
		actorType = _actorType;
		actorConf = ActorDataCollection.GetConf(actorId);
		...
		actorBodyPoints = GetComponentInChildren<ActorBodyPoints>();

		if (actorType == EmActorType.Monster)
		{
			ActorInfoTitleCtrl infoTitleCtrl = UIManager.instance.actorInfoTitlePanel.GetTitleCtrl();
			infoTitleCtrl.Init(this);
		}
		else if (actorType == EmActorType.Player)
		{
			UIManager.instance.mainUIPanel.SetPlayerHp((int)hp);
		}

		if (weaponId > 0)
			ChangeWeapon(WeaponDataCollection.GetConf(weaponId));
	}
	...
	void Update ()
	{
		...
		switch (myState)
		{
			...
			case EmActorState.WalkForward:
				...
				SetActorPosWithCollision(transform.localPosition + transform.forward * actorConf.walkForwardSpeed * Time.deltaTime);
				break;
			...
			case EmActorState.WalkLeft:
				transform.localRotation = Quaternion.Euler(0, lookRotation.y, 0);
				SetActorPosWithCollision(transform.localPosition - transform.right * actorConf.walkForwardSpeed * Time.deltaTime);
				break;
			...
			case EmActorState.Jump:
				jumpSpeed.y -= GRAVITATIONAL_ACCELERATION * Time.deltaTime;
				Vector3 pos = transform.position + jumpSpeed * Time.deltaTime;
			...
			case EmActorState.Attack:
				if (stateTimer >= actorConf.attackTimePoint && !hasDoAttack)
				{
			...
			case EmActorState.TossGrenade:
				if (stateTimer >= actorConf.attackTimePoint2 && !hasDoAttack)
				{
					DoAttack(actorConf.bulletId2);
					hasDoAttack = true;
				}
				if (stateTimer >= stateDuration)
				{
		...
		{
			//Vector3 euler = actorBodyPoints.spinePos.localEulerAngles;
			//euler.x += lookRotation.x;
			if(actorBodyPoints.runtimeSpinePos != null)
				actorBodyPoints.runtimeSpinePos.localRotation = Quaternion.Euler(new Vector3(lookRotation.x, 0 ,0));
			actorBodyPoints.bulletRoot.localRotation = Quaternion.Euler(new Vector3(lookRotation.x, 0, 0));
		}
	}
	...
	public void ChangeWeapon(WeaponConf weapon)
	{
		if (weaponModel != null)
		{
			Destroy(weaponModel);
		...
		weaponConf = weapon;
		weaponModel = GameObject.Instantiate(weaponConf.modelPrefab);
		weaponModel.transform.parent = actorBodyPoints.gunPos;
		weaponModel.transform.localRotation = Quaternion.identity;
		weaponModel.transform.localPosition = Vector3.zero;
		weaponModel.transform.localScale = Vector3.one;
		SetAllChildLayer(weaponModel.transform, gameObject.layer);
	}
